...
function PricesAndMaterials({ prices, loading, error }: { prices: PriceWithSubs[]; loading: boolean; error: string | null }) {
        ...
        <div className="grid lg:grid-cols-2 gap-10 lg:gap-12">
          ...
          <div id="prices-block" className="reveal">
            ...
            <div className="bg-gradient-to-br from-white/[0.07] to-white/[0.02] border border-white/10 rounded-2xl overflow-hidden">
              {loading && (
                <div className="px-6 py-10 text-center text-gray-400">Загрузка цен…</div>
              )}
              {error && !loading && (
                <div className="px-6 py-10 text-center text-red-400">Не удалось загрузить цены. Попробуйте позже.</div>
              )}
              {!loading && !error && prices.map((item, i) => {
